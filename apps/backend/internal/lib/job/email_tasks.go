...
func EnqueueReminderEmail (client *asynq.Client, task *ReminderEmailTask) error {
	...
	reminderTask := asynq.NewTask(TaskReminderEmail, payload,
		asynq.MaxRetry(3),
		asynq.Queue("default"),
		asynq.Timeout(30*time.Second))

	_, err = client.Enqueue(reminderTask)
	return err
}
...
func EnqueueWeeklyReportEmail(client *asynq.Client, task *WeeklyReportEmailTask) error {
	...
	reportTask := asynq.NewTask(TaskWeeklyReportEmail, payload,
		asynq.MaxRetry(3),
		asynq.Queue("default"),
		asynq.Timeout(30*time.Second))

	_, err = client.Enqueue(reportTask)
	return err
}
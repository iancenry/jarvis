...
func (j *JobService) handleReminderEmailTask(ctx context.Context, t *asynq.Task) error {
	...
	if err != nil {
		j.logger.Error().
			...
			Str("todo_id", p.TodoID.String()).
			Err(err).
			Msg("Failed to get user email for reminder")
		return fmt.Errorf("failed to resilve user email for user %s: %w", p.UserID, err)
	}
	...
	default:
		return fmt.Errorf("unknown reminder email task type: %s", p.TaskType)
	}
